---
title: "R script for SimNIBS simulated magnitude E-field values "
...
fontsize <- 28
fontsize_2 <- 26
ylab_text <- "mean magn E in V/m"
ylab_LMM_text <- "95% CI EMM in V/m"
```

# 1. Data preparation
## Read in tables
- Long format for simulated magnitude E values of actual and planned Electrodposition for each Montage and Area
```{r}
#magnitude meanE
M1_readout_M1_meanE_125 <- "/df_sphere_E_norm_fsaverage_mni_space_12.5_IFG_origin_IFG_M1_origin_M1_readoutrM1.feather"
M1_readout_M1_meanE_25 <- "/df_sphere_E_norm_fsaverage_mni_space_25_IFG_origin_IFG_M1_origin_M1_readoutrM1.feather"
#(Optional)
M1_readout_M1_meanE_375 <- "/df_sphere_E_norm_fsaverage_mni_space_37.5_IFG_origin_IFG_M1_origin_M1_readoutrM1.feather"


#Normal component positive part meanE

M1_readout_M1_normal_meanE_125 <- "/df_sphere_E_normal_fsaverage_mni_space_12.5_IFG_origin_IFG_M1_origin_M1_readoutrIFG.feather"
...
data_meanE_125 <- paste0(path_source_data, M1_readout_M1_meanE_125)
data_meanE_25 <- paste0(path_source_data, M1_readout_M1_meanE_25)
#(Optional)
data_meanE_375 <- paste0(path_source_data, M1_readout_M1_meanE_375)


df_meanE_125 <- arrow::read_feather(data_meanE_125)
df_meanE_25 <- arrow::read_feather(data_meanE_25)
#(Optional)
df_meanE_375 <- arrow::read_feather(data_meanE_375)
```
## create ROI radius columns
```{r}
View(df_meanE_25)

df_meanE_125$sphere <- "1.25"   #d = 2.5 mm
df_meanE_25$sphere <- "2.5"     #d = 5 mm
#(optional)
df_meanE_375$sphere <- "3.75"   #d = 7.5 mm
```

## Concatenate two data frames with different radii
```{r}
df <- rbind(df_meanE_125, df_meanE_25)
...
## Create index for random variable (every subject needs)
...
# Change column name
names(df_all)[names(df_all) == "mean magnitue E-field V/m"] <- "meanE"
names(df_all)
```
...
model_Mo_old <- lmer(meanE ~ 1 + Sex + Session + Treatment + Montage * sphere * coordinatespace * Area + (1 | lmm_group), data = df_all)
#summary(model_Mo_old)

# Correlated random intercept and slope for sphere
model_Mo_sphere <- lmer(meanE ~ 1 + Sex + Session + Treatment + Montage * sphere * coordinatespace * Area + (sphere | lmm_group), data = df_all)
#summary(model_Mo)

# Correlated random intercept and slope for electrode position
model_Mo_coo <- lmer(meanE ~ 1 + Sex + Session + Treatment + Montage * sphere * coordinatespace * Area + (coordinatespace  | lmm_group), data = df_all)
#summary(model_Mo)

# Correlated random intercept and slope for sphere and electrode position
model_Mo <- lmer(meanE ~ 1 + Sex + Session + Treatment + Montage * sphere * coordinatespace * Area + (sphere + coordinatespace | lmm_group), data = df_all)
summary(model_Mo)
```
...
```{r}
# Choose winning model with lowest AIC and BIC
anova(model_Mo, model_Mo_sphere, model_Mo_coo, model_Mo_old)
```
...
# generate R² values also shown in Table 4
