...
g <- ggplot() + 
    ...
    geom_violinhalf(data = df,
        aes(x = !!Interaction, y = meanE, fill = !!Interaction),
        flip = c(1, 3), trim = FALSE, alpha = 0.3) +
        scale_fill_grey(start = 0.01,
        end = 0.6) +
    geom_point(data = df,
         aes(x = !!Interaction, y = meanE, color = !!Interaction),
   ...
   theme_classic() +
    ...
    if (Numb  == "4") {
     ...
     axis.title.x = element_blank())+
      ...
      position = "right", limits = c(0, 0.35)) +
      theme(axis.text.y.right = element_text(color = COLOR),
    ...
    panel.border = element_rect(colour = "black", fill = NA, size = 1))
     ...
     axis.title.y = element_blank()) +
    scale_y_continuous(name = ylab_text, limits = c(0, 0.35)) +
...
plot_posthoc_multi_all <- function(df, emm, Interaction, 
Factor1, level1, Factor2, level2, Factor3, level3,
Numb, xtitle, facet1, facet2, facet3, COLOR)

{Interaction <- enquo(Interaction)
 facet1 <- enquo(facet1)
 facet2 <- enquo(facet2)
 facet3 <- enquo(facet3)
df <- df[df[[Factor1]] == level1 & df[[Factor2]] == level2  & df[[Factor3]] == level3,]
emm <- emm[emm[[Factor1]] == level1 & emm[[Factor2]] == level2 & emm[[Factor3]] == level3,]

g <- ggplot() +
  ylim(0, 0.35) +
   theme_classic() +
    theme(panel.grid = element_line(size = 0.5, linetype = 'solid',
                                colour = "grey")) + 
    geom_violinhalf(data = df,
        aes(x = !!Interaction, y = meanE, fill = !!Interaction),
        flip = c(1, 3), trim = FALSE, alpha = 0.3) +
    ...
    geom_errorbar(data = emm,
        ...
        width = 0.2, linewidth = 2, color = COLOR,
    position = position_dodge(0.8)) +
    theme(text = element_text(family = "DejaVuSans", size = 20)
        ) +
     theme(legend.position = "none")
     if (Numb  == "1")      {
     g <- g +
     ...
     axis.title.x = element_blank(),
     axis.line.x = element_blank()) +
    scale_y_continuous(name = "Mean E V/m", limits = c(0, 0.35)) +
    facet_wrap(c(facet1, facet2), scales = "free") +
    theme(panel.grid = element_line(), panel.grid.major = element_line(),
    panel.border = element_rect(colour = "black", fill = NA, size = 1))
    } else if (Numb  == "2") {
     g <- g +
    ...
    panel.border = element_rect(colour = "black", fill = NA, size = 1))
     ...
     axis.title.y = element_blank(),
     axis.line.y = element_blank()) +
     scale_y_continuous(name = "95% CI EMM V/m",
     position = "right", limits = c(0, 0.35)) +
      theme(axis.text.y.right = element_text(color = COLOR),
      axis.title.y.right = element_text(colour = COLOR)) +
      facet_wrap(c(facet3, facet1, facet2), scales = "free",
      strip.position = c("top", "top", "right"))  +
      facet_grid(vars(!!facet1), vars(!!facet3)) +
    theme(panel.grid = element_line(), panel.grid.major = element_line(),
    panel.border = element_rect(colour = "black", fill = NA, size = 1))
    } else if (Numb == "3") {
     g <- g +
    ...
    theme(legend.position = "none") +
      ...
      theme(axis.text.y.right = element_text(color = COLOR),
      axis.title.y.right = element_text(colour = COLOR)) +
      facet_grid(vars(!!facet3), scales = "free") +
      xlab(xtitle) +
    theme(panel.grid = element_line(), panel.grid.major = element_line(),
    panel.border = element_rect(colour = "black", fill = NA, size = 1))}
     return(g)
