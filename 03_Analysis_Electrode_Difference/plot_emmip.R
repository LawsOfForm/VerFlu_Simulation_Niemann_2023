emms_generate <- function(model,path,A,B,C){
Limit <- 25000
    if (missing(C) & !(missing(B))) {
    f <- as.formula(paste0(A,"~", B));
    emm_f <- as.formula(paste0("pairwise ~ ", A, "|",B));
    Parameter <- paste0(A,"_",B)
    
        if (A == "Electrode" ) { reduce <- -6
        
        }
        else if (B == "Electrode") { reduce <- -3
    ...
    } else if (missing(B) & missing(C)) {
    f <- as.formula(paste0( "~", A));
    emm_f <- as.formula(paste0("pairwise ~ ", A));
    Parameter <- A
    
        if (A == "Electrode") { reduce <- -3
...
plot_posthoc <- function(df, emm, Interaction, COLOR)
...
g <- ggplot() + 
    ...
    geom_violinhalf(data = df,
        aes(x = !!Interaction, y = diff, fill = !!Interaction),
        flip = c(1, 3), trim = FALSE, alpha = 0.3) +
        scale_fill_grey(start = 0.01,
        end = 0.6) +
    geom_point(data = df,
         aes(x = !!Interaction, y = diff, color = !!Interaction),
...
plot_posthoc_multi <- function(df, emm, Interaction, Factor1, level1, Factor2, level2,
Numb, xtitle, facet1, facet2, COLOR, Angle)
{Interaction <- enquo(Interaction)
...
g <- ggplot() + 
    ...
    scale_y_continuous(name = ylab_text, limits = c(limm, limp)) +
    ...
    panel.border = element_rect(colour = "black", fill = NA, size = 1)) 
     ...
     position = "right", limits = c(limm, limp)) +
      theme(axis.text.y.right = element_text(color = COLOR),
      axis.title.y.right = element_text(colour = COLOR))  +
