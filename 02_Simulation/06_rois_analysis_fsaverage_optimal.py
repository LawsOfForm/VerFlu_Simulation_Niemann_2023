#Issue: Get mean E-field images and images for Figure 3 and Figure S5 for plannes electrodes
...
if pathmanager.Exp == "VerFlu_HI" or pathmanager.Exp == "VerFlu_HM":
    folder = "simulations_mni_indiv_radius"
    
elif pathmanager.Exp == "VerFlu_TI" or pathmanager.Exp == "VerFlu_TM" or pathmanager.Exp == "VerFlu_TM":
    folder = "simulations_mni"

for Session in ['Kopf','Zahl']:
    sheet= pathmanager.Exp +'_' + Session

    stim_data_cor = pd.read_excel("./Source_Data/Electrode_Coordinates_full_coregistr_sub2mni.xlsx", sheet_name=sheet, engine="openpyxl")
    ...
    stim_data_cor=(stim_data_cor
                .set_index('id')
                .drop(outlier_list, errors = 'ignore')
                )

    for sub in stim_data_cor.index:
    #only use subjects with 
        if not os.path.isfile(os.path.join(pathmanager.BASEDIRECTORY, sub, f"ses-{Session}", "SimNIBS", folder,"fsavg_overlays", f'{sub}_TDCS_1_scalar_fsavg.msh')):
            #get outlier
            ...
            results_fsavg = simnibs.read_msh(os.path.join(pathmanager.BASEDIRECTORY, sub, f"ses-{Session}", "SimNIBS", folder,"fsavg_overlays", f"{sub}_TDCS_1_scalar_fsavg.msh"))
...
# Plot
    ...
    #print only the positive average values of the whole brain
    ...
    ## Plot the ROI
    results_fsavg.add_node_field(roi, region)
    ## uncomment code below is you want to see mesh file
    #results_fsavg.view(visible_fields='roi').show()
